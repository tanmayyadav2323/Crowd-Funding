...
import "./chunk-M4ZHEDAY.js";
import "./chunk-J3SRJP3Z.js";
import "./chunk-E3TFATJ7.js";
import "./chunk-UJM344VD.js";
import "./chunk-ASYL3OTN.js";
import "./chunk-2BBPEYBM.js";
import "./chunk-IASXPJJQ.js";
import "./chunk-ZYG6HVUT.js";
import "./chunk-FEAJ3HXH.js";
import {
  LOCAL_NODE_PKEY,
  ThirdwebSDK,
  getChainProvider
...
var import_utils2 = __toESM(require_src());
function toJSON(op) {
...
async function getUserOpHashV06(userOp, entryPoint, chainId) {
  const op = await (0, import_utils.resolveProperties)(userOp);
  const hashedUserOp = {
    sender: op.sender,
    nonce: op.nonce,
    initCodeHash: (0, import_utils.keccak256)(op.initCode),
    callDataHash: (0, import_utils.keccak256)(op.callData),
    callGasLimit: op.callGasLimit,
    verificationGasLimit: op.verificationGasLimit,
    preVerificationGas: op.preVerificationGas,
    maxFeePerGas: op.maxFeePerGas,
    maxPriorityFeePerGas: op.maxPriorityFeePerGas,
    paymasterAndDataHash: (0, import_utils.keccak256)(op.paymasterAndData)
  };
  const userOpType = {
    components: [{
      type: "address",
  ...
  let encoded = import_utils.defaultAbiCoder.encode([userOpType], [{
    ...hashedUserOp
  }]);
  const userOpHash = (0, import_utils.keccak256)(encoded);
  const enc = import_utils.defaultAbiCoder.encode(["bytes32", "address", "uint256"], [userOpHash, entryPoint, chainId]);
  return (0, import_utils.keccak256)(enc);
}
var SIG_SIZE = 65;
...
var VerifyingPaymasterAPI = class extends import_sdk.PaymasterAPI {
  ...
  async getPaymasterAndData(userOp) {
    ...
    try {
      ...
      console.error("PM - error", ((_a = e.result) == null ? void 0 : _a.error) || e);
      throw e;
    }
  }
};
var getVerifyingPaymaster = (paymasterUrl, entryPoint, apiKey) => new VerifyingPaymasterAPI(paymasterUrl, entryPoint, apiKey);
var UserOperationEventListener = class {
  ...
  async extractFailureReason(receipt) {
    ...
    if (revertReasonEvents[0]) {
      let message = revertReasonEvents[0].args.revertReason;
      if (message.startsWith("0x08c379a0")) {
        message = import_utils.defaultAbiCoder.decode(["string"], "0x" + message.substring(10)).toString();
      }
      this.reject(new Error(`UserOp failed with reason: ${message}`));
    }
  }
...
var ERC4337EthersSigner = class extends Signer {
  constructor(config, originalSigner, erc4337provider, httpRpcClient, smartAccountAPI) {
    ...
    (0, import_utils.defineReadOnly)(this, "provider", erc4337provider);
    this.config = config;
    this.originalSigner = originalSigner;
    this.erc4337provider = erc4337provider;
    this.httpRpcClient = httpRpcClient;
  ...
  async sendTransaction(transaction) {
    ...
    await this.verifyAllNecessaryFields(tx);
    const userOperation = await this.smartAccountAPI.createSignedUserOp({
      target: (_a = tx.to) != null ? _a : "",
      data: (_c = (_b = tx.data) == null ? void 0 : _b.toString()) != null ? _c : "",
      value: tx.value,
      gasLimit: tx.gasLimit
    }, batched);
    const transactionResponse = await this.erc4337provider.constructUserOpTransactionResponse(userOperation);
  ...
  unwrapError(errorIn) {
    ...
    try {
      ...
      if (errorIn.body && typeof errorIn.body === "object") {
        const errorBody = JSON.parse(errorIn.body);
        let paymasterInfo = "";
        let failedOpMessage = ((_a = errorBody == null ? void 0 : errorBody.error) == null ? void 0 : _a.message) || ((_b = errorBody == null ? void 0 : errorBody.error) == null ? void 0 : _b.data);
        if ((failedOpMessage == null ? void 0 : failedOpMessage.includes("FailedOp")) === true) {
          const matched = failedOpMessage.match(/FailedOp\((.*)\)/);
          if (matched) {
            const split = matched[1].split(",");
            paymasterInfo = `(paymaster address: ${split[1]})`;
...
var ERC4337EthersProvider = class extends ethers_exports.providers.BaseProvider {
  constructor(chainId, config, originalSigner, originalProvider, httpRpcClient, entryPoint, smartAccountAPI) {
    super({
      name: "ERC-4337 Custom Network",
      chainId
  ...
  async constructUserOpTransactionResponse(userOp1) {
    const userOp = await (0, import_utils.resolveProperties)(userOp1);
    ...
    return {
      ...
      data: (0, import_utils.hexValue)(userOp.callData),
      chainId: this.chainId,
      wait: async (confirmations) => {
...
var HttpRpcClient = class {
  ...
  async sendUserOpToBundler(userOp1) {
    await this.initializing;
    const hexifiedUserOp = (0, import_utils2.deepHexlify)(await (0, import_utils.resolveProperties)(userOp1));
    const jsonRequestData = [hexifiedUserOp, this.entryPointAddress];
    await this.printUserOperation("eth_sendUserOperation", jsonRequestData);
    return await this.userOpJsonRpcProvider.send("eth_sendUserOperation", [hexifiedUserOp, this.entryPointAddress]);
  }
...
var BaseAccountAPI = class {
  ...
  async getPreVerificationGas(userOp) {
    const p = await (0, import_utils.resolveProperties)(userOp);
    return (0, import_sdk.calcPreVerificationGas)(p, this.overheads);
  }
  packUserOp(userOp) {
    return (0, import_utils2.packUserOp)(userOp, false);
  }
  ...
  async createUnsignedUserOp(info, batched) {
    ...
    if (!maxFeePerGas || !maxPriorityFeePerGas) {
      const feeData = await this.provider.getFeeData();
      if (!maxFeePerGas) {
        maxFeePerGas = (_a = feeData.maxFeePerGas) != null ? _a : void 0;
      ...
      if (!maxPriorityFeePerGas) {
        maxPriorityFeePerGas = (_b = feeData.maxPriorityFeePerGas) != null ? _b : void 0;
      }
    }
    const partialUserOp = {
      sender: this.getAccountAddress(),
...
var AccountAPI = class extends BaseAccountAPI {
  ...
  async getAccountInitCode() {
    ...
    return (0, import_utils.hexConcat)([factory.getAddress(), tx.encode()]);
  }
  ...
  async signUserOpHash(userOpHash) {
    return await this.params.localSigner.signMessage((0, import_utils.arrayify)(userOpHash));
  }
...
var SmartWalletConnector = class extends Connector {
  ...
  async initialize(personalWallet) {
    ...
    const providerConfig = {
      ...
      entryPointAddress,
      bundlerUrl,
      paymasterAPI: this.config.gasless ? this.config.paymasterAPI ? this.config.paymasterAPI : getVerifyingPaymaster(paymasterUrl, entryPointAddress, this.config.thirdwebApiKey) : void 0,
...
//# sourceMappingURL=thirdweb-dev-wallets-evm-connectors-smart-wallet.browser.esm-U5ZQ4R74.js.map
